...
export function useTickets(): TicketViewState {
  ...
  const createLot = useCallback(
    async (input: CreateTicketLotInput) =>
      run(() => window.gtrz.tickets.createLot(input), 'Lote criado.'),
    [run],
  );
  const updateLot = useCallback(
    async (input: UpdateTicketLotInput) =>
      run(() => window.gtrz.tickets.updateLot(input), 'Lote atualizado.'),
    [run],
  );
  const createSale = useCallback(
    async (input: CreateTicketSaleInput) =>
      run(() => window.gtrz.tickets.createSale(input), 'Ingressos registrados.'),
    [run],
  );
  const cancelSale = useCallback(
    async (saleId: string, reason: string) =>
      run(() => window.gtrz.tickets.cancelSale({ saleId, reason }), 'Venda cancelada.'),
    [run],
  );
  const deleteSale = useCallback(
    async (saleId: string, reason: string) =>
      run(
        ...
        'Venda e ingressos excluídos definitivamente.',
      ),
    [run],
  );
  return {
