...
export function VoucherEditForm({ voucher, tables, busy, onCancel, onSubmit }: VoucherEditFormProps): React.JSX.Element {
  ...
  return (
    ...
    >
      <label className="form-field">
        ...
        <input disabled={busy} maxLength={100} onChange={(event) => setLabel(event.target.value)} required value={label} />
      </label>
      <label className="form-field">
        <span>Código</span>
        <input disabled={busy} maxLength={32} onChange={(event) => setCode(event.target.value.toLocaleUpperCase('pt-BR'))} required value={code} />
      </label>
      <label className="form-field">
        <span>Mesa vinculada</span>
        <select disabled={busy || linkedTableStillExists} onChange={(event) => setServicePointId(event.target.value)} required value={servicePointId}>
      ...
      <label className="form-field">
        ...
        <input disabled={busy} inputMode="decimal" onChange={(event) => setAddBalance(event.target.value)} placeholder="0,00" value={addBalance} />
      ...
      <div className="voucher-edit-form__actions">
        <button className="button button--ghost button--compact" disabled={busy} onClick={onCancel} type="button"><X size={15} aria-hidden="true" />Fechar</button>
        <button className="button button--compact" disabled={busy || label.trim().length < 2 || code.trim().length < 4 || servicePointId.length === 0} type="submit"><Save size={15} aria-hidden="true" />Salvar alterações</button>
      </div>
    </form>
  );
}
