  ...
  return (
    ...
    >
      ...
      <label className="form-field">
        ...
        <input disabled={busy} maxLength={100} onChange={(event) => setLabel(event.target.value)} placeholder="Ex.: Crédito patrocinador" required value={label} />
      </label>
      <label className="form-field">
        <span>Código opcional</span>
        <input disabled={busy} maxLength={32} onChange={(event) => setCode(event.target.value.toLocaleUpperCase('pt-BR'))} placeholder="Gerado automaticamente" value={code} />
      </label>
      <label className="form-field">
        <span>Saldo inicial</span>
        <input disabled={busy} inputMode="decimal" onChange={(event) => setBalance(event.target.value)} placeholder="100,00" required value={balance} />
      </label>
      <label className="form-field">
        <span>Mesa vinculada</span>
        <select disabled={busy} onChange={(event) => setServicePointId(event.target.value)} required value={servicePointId}>
      ...
      <button className="button" disabled={busy || label.trim().length < 2 || parseMoney(balance) <= 0 || servicePointId.length === 0} type="submit">
        Emitir voucher
      </button>
    </form>
  );
