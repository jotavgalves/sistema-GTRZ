...
function requireExpense(database: DatabaseContext, expenseId: string): ExpenseRow {
  const row = database.sqlite
    .prepare(
      `SELECT id,event_id,category,description,amount_cents,payment_method,note,status,created_at,cancelled_at,updated_at FROM expenses WHERE id=?`,
    )
    .get(expenseId) as ExpenseRow | undefined;
  if (row === undefined) throw new Error('A despesa informada não existe.');
  return row;
}
export function getExpenseState(database: DatabaseContext): DatabaseExpenseState {
  ...
  if (eventId === null) return { activeEventId: null, expenses: [] };
  const rows = database.sqlite
    .prepare(
      `SELECT id,event_id,category,description,amount_cents,payment_method,note,status,created_at,cancelled_at,updated_at FROM expenses WHERE event_id=? ORDER BY CASE status WHEN 'active' THEN 0 ELSE 1 END,updated_at DESC`,
    )
...
export function createExpense(
  ...
  database.sqlite.transaction(() => {
    database.sqlite
      .prepare(
        `INSERT INTO expenses(id,event_id,category,description,amount_cents,payment_method,note,status,created_at,cancelled_at,updated_at) VALUES(?,?,?,?,?,?,?,'active',?,NULL,?)`,
      )
      .run(
        expenseId,
        eventId,
...
export function cancelExpense(
  ...
  requireProduction(database);
  const eventId = requireActiveEvent(database),
    expense = requireExpense(database, input.expenseId);
  if (expense.event_id !== eventId) throw new Error('A despesa não pertence ao evento ativo.');
  if (expense.status === 'cancelled') throw new Error('Esta despesa já foi cancelada.');
  const reason = input.reason.trim(),
    now = Date.now();
  database.sqlite.transaction(() => {
    database.sqlite
      .prepare(`UPDATE expenses SET status='cancelled',cancelled_at=?,updated_at=? WHERE id=?`)
    ...
    appendAudit(database, {
      ...
      entityId: expense.id,
      eventId,
      details: { amountCents: expense.amount_cents, description: expense.description, reason },
    });
  })();
