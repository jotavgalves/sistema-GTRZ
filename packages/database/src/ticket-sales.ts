...
export function createTicketSale(
  ...
  if (!lot.active) throw new Error('O lote está inativo e não aceita novas vendas.');
  ...
  database.sqlite.transaction(() => {
    database.sqlite
      .prepare(
        `INSERT INTO ticket_sales (id,event_id,lot_id,lot_name,attendee_name,source,quantity,unit_price_cents,total_cents,payment_method,status,created_at,cancelled_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,'active',?,NULL,?)`,
      )
      .run(
        saleId,
        eventId,
    ...
    const insertCode = database.sqlite.prepare(
      `INSERT INTO ticket_codes (id,event_id,sale_id,code,status,created_at) VALUES (?,?,?,?,'valid',?)`,
    );
    for (const code of codes) insertCode.run(randomUUID(), eventId, saleId, code, now);
...
export function cancelTicketSale(
  ...
  if (sale.event_id !== eventId) throw new Error('A venda não pertence ao evento ativo.');
  if (sale.status === 'cancelled') throw new Error('Esta venda de ingresso já foi cancelada.');
  const reason = input.reason.trim();
  const now = Date.now();
  database.sqlite.transaction(() => {
    database.sqlite
      .prepare(`UPDATE ticket_sales SET status='cancelled',cancelled_at=?,updated_at=? WHERE id=?`)
      .run(now, now, sale.id);
    database.sqlite
...
export function deleteTicketSale(
  ...
  if (wasCancelledFirst) {
    cancelTicketSale(database, input);
    sale = requireTicketSale(database, input.saleId);
  }
